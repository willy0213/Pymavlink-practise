...
# run until all fence items are set successfully
while idx < len(fence_list):
    ...
    # create FENCE_FETCH_POINT_MESSAGE
    ...
    latitute = message["lat"]
    longitude = message["lng"]

    # check the fence pount is uooladed successfully
    if latitute != 0.0 and longitude != 0.0:

        # increment the index of the fence item
        idx += 1

# check all fence items are set successfully
print("All fence items set successfully")
...
while True:
    ...
    if message["param_id"] == "FENCE_ACTION":
        ...
        # should send param set message again
            ...
            print("Faile to set FENCE_ACTION set to original value {0} successfully".format(fence_action_original))
